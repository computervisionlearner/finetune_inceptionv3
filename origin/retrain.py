...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  subdirs =sorted([ subdir[0] for subdir in os.walk(image_dir)])
  result = {}
  for subdir in subdirs[1:]:
    file_list = sorted(glob.glob(os.path.join(subdir,'*.jpg')))
    ...
    for file in file_list:
      ...
      if choice < testing_percentage:
        test_images.append(basename)
      elif choice < testing_percentage + validation_percentage:
        valid_images.append(basename)
      else:
        train_images.append(basename)
    result[label_name] = {
                          'dir':label_name, 'validation': valid_images,
                          'testing': test_images, 'training': train_images
                          }
  return result
